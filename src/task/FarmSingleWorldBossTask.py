...
class FarmSingleWorldBossTask(BaseCombatTask):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.description = "Click Start in Game World"
        self.name = "Farm Single World Boss"
        ...
        default_config = {
            'Boss': 'N/A',
            'Boss Echo Pickup Method': 'Turn Around and Search',
            'Boss Respawn Time': 150 # 2 minute 30 second        
        }
        self.config_type['Boss Echo Pickup Method'] = {'type': "drop_down", 'options': self.find_echo_method}
        default_config.update(self.default_config)
        self.default_config = default_config
        self.config_type["Boss"] = {'type': "drop_down", 'options': self.boss_names}
        ...
        while True:
            if boss_name := self.config.get("Boss"):
                if boss_name != 'N/A':
                    ...
                    method = self.config.get(f'Boss Echo Pickup Method', 'Walk')
                    ...
                    # Cooldown
                    self.boss_cooldown(self.config.get("Boss Respawn Time"), self.config.get("Boss"))
                else:
                    self.log_error('Set a boss name to start!', notify=True)
                    return

    def boss_cooldown(self, time, boss):
        self.teleport_to_boss(boss)
        self.sleep(time) 
